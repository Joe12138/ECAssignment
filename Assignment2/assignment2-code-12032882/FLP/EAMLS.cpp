...
void EAMLS::init_pop(int p_size, int n_site) {
     ...
     * @return: 更改了全局变量 this->pop 和 this->index_m_map
     */
    ...
    for(int i=0; i<p_size; i++){
        ...
        m_value = EAMLS::check_constraint(solution);
        if(m_value < 2){
            if(this->repair_lan){
                this->repair_strategy_lan(solution, m_value);
            }else{
...
void EAMLS::init_pop_lan(int p_size, int n_site) {
    /**
     * 我提出的初始种群方法
     * @param p_size: the size of the initialisation size
     * @param n_site: the number of site in this instance
     * @return: change global variable this->pop and this->index_fitness_map
     */

    // the solution which all bits are zero.
    ...
    default_random_engine generator(clock());
//    generator.seed(clock());
    // this ratio need to be thought carefully.
...
int EAMLS::readjust_single_solution_order(const vector<unsigned char> &solution,
    ...
    /**
     * Given a solution, obtain candidate locations order for each site.
     */
...
void EAMLS::memorable_local_search(unordered_map<int, vector<unsigned char>> &parent_pop,
                                   ...
                                   unordered_set<vector<unsigned char>, hash_vector> &ind_local_search,
                                   unordered_map<int, double> &parent_index_fitness_map,
                                   unordered_map<int, double> &mutation_index_fitness_map,
                                   unordered_set<vector<unsigned char>, hash_vector> &ls_offspring) {
//    unordered_set<vector<unsigned char>, hash_vector >ls_offspring;
...
//            unordered_set<vector<unsigned char>, hash_vector> neighbor_inds;
...
void EAMLS::get_all_neighbor(const vector<unsigned char> &solution,
                             unordered_set<vector<unsigned char>, hash_vector> &neighbor_set) {
    unsigned char one = 1;
    unsigned char zero = 0;
    for(int i=0; i<solution.size(); i++){
...
double EAMLS::get_l3_value(unordered_map<int, vector<unsigned char>> &pop_after_selection,
                           unordered_set<vector<unsigned char>, hash_vector> &all_neighbor_inds) {
    int num = 0;
...
void EAMLS::survival_selection(unordered_map<int, vector<unsigned char>> &mutation_pop,
                               unordered_set<vector<unsigned char>, hash_vector> &ls_pop,
...
double EAMLS::EA_with_MLS(int generation_num, double mutation_rate, double beta, int step_size, int n) {
    string file_name = R"(E:\2021_spring\Evolutional Computing\ECAssignment\Assignment2\FLP\Result\Detail\50_Neighborhood_True\50_Neighbor_True_)" +
    ...
    unordered_set<vector<unsigned char>, hash_vector> all_neighbor_inds;
    unordered_set<vector<unsigned char>, hash_vector> indLSed;
